...
class nnUNetTrainerV2_GAN(nnUNetTrainerV2):
    """
    ...
    def run_training(self):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            if self.use_progress_bar:
                with trange(self.num_batches_per_epoch) as tbar:
                    for b in tbar:
                        ...
                        l, l_gen, l_ad, _ = self.run_iteration(self.tr_gen, True)

                        tbar.set_postfix(loss=l)
                        train_losses_epoch.append(l)
                        gen_losses_epoch.append(l_gen)
            ...
            self.all_tr_gen_losses.append(np.mean(gen_losses_epoch))
            self.print_to_log_file("gen loss : %.4f" % self.all_tr_gen_losses[-1])
            ...
            with torch.no_grad():
                ...
                for b in range(self.num_val_batches_per_epoch):
                    l, l_gen, l_ad, ad_acc = self.run_iteration(self.val_gen, False, True)
                    val_losses.append(l)
                    val_ad_losses.append(l_ad)
                    val_gen_losses.append(l_gen)
                    val_ad_acc.append(ad_acc)
                self.all_val_losses.append(np.mean(val_losses))
                self.all_val_ad_losses.append(np.mean(val_ad_losses))
                self.all_val_gen_losses.append(np.mean(val_gen_losses))
                ...
                if self.also_val_in_tr_mode:
                    ...
                    val_ad_acc = []
                    for b in range(self.num_val_batches_per_epoch):
                        l, l_gen, l_ad, ad_acc = self.run_iteration(self.val_gen, False)
                        val_losses.append(l)
                        val_ad_losses.append(l_ad)
                        val_gen_losses.append(l_gen)
                        val_ad_acc.append(ad_acc)
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        if self.fp16:
            ...
            with autocast(): 
                ...
                gen_pred = output = self.network(data)               
                ...
                del data
                # gen_loss = self.loss(output, target)
                gen_loss = self.loss(gen_pred, target)
                l = gen_loss - dis_loss
                print("Forward pass time:", time.time() - f_start, "seconds.")
                
            if do_backprop:
                b_start = time.time()
                self.amp_grad_scaler.scale(l).backward()
                self.amp_grad_scaler.unscale_(self.optimizer)
                self.amp_grad_scaler.unscale_(self.ad_optimizer)
                torch.nn.utils.clip_grad_norm_(self.network.parameters(), 12)
                # torch.nn.utils.clip_grad_norm_(self.network.parameters(), 12)  ## clip adversary grad?
                self.amp_grad_scaler.step(self.optimizer)
                self.amp_grad_scaler.step(self.ad_optimizer)

                # update scaler once
                self.amp_grad_scaler.update()
                print("Backward pass time:", time.time() - b_start, "seconds.") 
        ...
        return l.detach().cpu().numpy(), gen_loss.detach().cpu().numpy(), dis_loss.detach().cpu().numpy(), dis_acc.cpu().numpy()
    ...
    def plot_progress(self):
        ...
        try:
            ...
            fig = plt.figure(figsize=(30, 24*3))
            ax1 = fig.add_subplot(311)  # combined
            ax2 = fig.add_subplot(312)  # disc
            ax3 = fig.add_subplot(313)  # gen
            ax1_2 = ax1.twinx()
            ax2_2 = ax2.twinx()
            ax3_2 = ax3.twinx()
            ...
            ax1.plot(x_values, self.all_tr_losses, color='b', ls='-', label="loss_tr")
            ax2.plot(x_values, self.all_tr_ad_losses, color='c', ls='-', label="loss_tr_ad")
            ax3.plot(x_values, self.all_tr_gen_losses, color='m', ls='-', label="loss_tr_gen")

            ax1.plot(x_values, self.all_val_losses, color='r', ls='-', label="loss_val, train=False")
            ax2.plot(x_values, self.all_val_ad_losses, color='r', ls='-', label="loss_val_ad, train=False")
            ax3.plot(x_values, self.all_val_gen_losses, color='r', ls='-', label="loss_val_gen, train=False")

            if len(self.all_val_losses_tr_mode) > 0:
                ax1.plot(x_values, self.all_val_losses_tr_mode, color='g', ls='-', label="loss_val, train=True")  # combined
                ax3.plot(x_values, self.all_val_losses_tr_mode, color='g', ls='-', label="loss_val, train=True")  # gen
            if len(self.all_val_eval_metrics) == len(x_values):
                ax1_2.plot(x_values, self.all_val_eval_metrics, color='g', ls='--', label="evaluation metric")  # combined
                ax3_2.plot(x_values, self.all_val_eval_metrics, color='g', ls='--', label="evaluation metric")  # gen

            if len(self.all_val_ad_acc) == len(x_values):
                ax2_2.plot(x_values, self.all_val_ad_acc, color='g', ls='--', label="acc_ad metric")  # gen
            ...
            ax2.set_ylabel("loss")
            ax2_2.set_ylabel("evaluation metric")
            ax2.legend()
            ax2_2.legend(loc=9)

            ax3.set_xlabel("epoch")
            ax3.set_ylabel("loss")
            ax3_2.set_ylabel("evaluation metric")
            ax3.legend()
            ax3_2.legend(loc=9)
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        ...
        save_this = {
            ...
            'plot_stuff': (self.all_tr_losses, self.all_val_losses, self.all_val_losses_tr_mode,
                           self.all_val_eval_metrics, self.all_tr_ad_losses, self.all_tr_gen_losses, self.all_val_ad_losses, 
                           self.all_val_gen_losses, self.all_val_ad_acc),
    ...
    def load_checkpoint_ram(self, checkpoint, train=True):
        ...
        self.all_tr_losses, self.all_val_losses, self.all_val_losses_tr_mode, self.all_val_eval_metrics, \
            self.all_tr_ad_losses, self.all_tr_gen_losses, self.all_val_ad_losses, self.all_val_gen_losses, \
            self.all_val_ad_acc = checkpoint['plot_stuff']
