...
def register_to_target(template, target, base, side, save_dir, downsample=False, downsample_size=300, flip=False):
	"""Summary
	...
	affine_path = adjust_file_path(save_dir, "%s %s %s" % (side, template, target), ".mat", registration='affine',
                                downsample=downsample, downsample_size=downsample_size, flip=flip)
	deform_path = adjust_file_path(save_dir, "%s %s %s" % (side, template, target), ".nii.gz", registration='forward',
                                downsample=downsample, downsample_size=downsample_size, flip=flip)
	...
	if downsample:
		...
		transform_forward = ants.registration(fixed=target_image_downsample, moving=template_image_downsample,
	...
	for target in target_scan_id:
		if template in target: 
			...
			if args['overwrite'] or not(os.path.exists(deform_path) and os.path.exists(affine_path)):
				register_to_target(template, target, base, side, save_dir, downsample=args['downsample'], downsample_size=args['downsample_size'], flip=args['flip'])
